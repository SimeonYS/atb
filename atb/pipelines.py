...
class AtbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('atb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `atb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM atb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO atb (date, title, link, content)"
